package com.seglino.jingyi.webapi;
...
public class HtmlToJson {
	...
	private JSONObject convertNodeToJsonObject(Node node, String tag, boolean needClass) {
		/*
		 * if(isScriptNode(tag)){ return null; }
		 */
		if (isTextNode(tag)) {
		...
		// 如果这个元素没有内容 忽略掉
		/*
		 * if (elementIsEmpty(element)) { return null; }
		 */
	...
	/**
	 * 判断空元素 条件是 没有文本型内容 不包含图片、视频、音频数据、还没有Style或者class的标签
	 * 
	 * @param element
	 * @return
	 */
	private boolean elementIsEmpty(Element element) {
		return element.hasText() == false && element.selectFirst("img") == null && element.selectFirst("video") == null && element.selectFirst("audio") == null && element.hasAttr("style") == false
...
//	public static class RenderedFactory implements StyleRendererFactory {
